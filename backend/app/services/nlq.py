...
class NaturalLanguageQueryService:
    ...
    def _parse_intent(self, query_text: str) -> tuple[str, Dict[str, Any]]:
        ...
        item_patterns = [
            ...
            r'where.+is.+(PALT-\d+|ITEM-\d+|[A-Z]+-\d+)',
        ...
        # Check inventory summary patterns
        summary_patterns = [
            r'(?:库存|inventory).+(?:总览|summary|概览)',
            r'current.+inventory',
            r'总共有多少'
        ]
    ...
    def _handle_bin_query(self, bin_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not item_ids:
                answer = f"Bin {bin_id} appears to be empty (last scanned: {latest_snapshot.ts.strftime('%Y-%m-%d %H:%M')})."
            else:
                answer = f"Bin {bin_id} contains {len(item_ids)} items: {', '.join(item_ids[:5])}{'...' if len(item_ids) > 5 else ''} (last scanned: {latest_snapshot.ts.strftime('%Y-%m-%d %H:%M')})."
            ...
            return {
                ...
                "data": {
                    "bin_id": bin_id,
                    "items": item_ids,
                    "last_scanned": latest_snapshot.ts.isoformat(),
                    "photo_url": photo_url,
